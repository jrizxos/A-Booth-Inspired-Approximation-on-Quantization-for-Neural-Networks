...
##### Pretrained Network ################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        DenseNet.train()
        train(DenseNet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'main'], load=True, device=DEVICE)
    DenseNet.eval()
    test_nn(DenseNet, validation_loader, device=DEVICE)
