...
##### Pretrained Network ################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        Mobilenet.train()
        train(Mobilenet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'main'], load=False, device=DEVICE)
    Mobilenet.eval()
    test(Mobilenet, validation_loader, device=DEVICE)
...
##### PTQ ###############################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        criterion = nn.CrossEntropyLoss()
        q_Mobilenet.train()
        train(q_Mobilenet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'quantizable'], load=False, device=DEVICE)
