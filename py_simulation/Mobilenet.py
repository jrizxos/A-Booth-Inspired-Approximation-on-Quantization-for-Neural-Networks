...
##### Pretrained Network ################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        optimizer = optim.SGD(Mobilenet.parameters(), lr=0.01, momentum=0.5)
        criterion = nn.CrossEntropyLoss()
        Mobilenet.train()
        train(Mobilenet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'main'], load=True, device=DEVICE)
    Mobilenet.eval()
    test_nn(Mobilenet, validation_loader, device=DEVICE)
...
##### PTQ ###############################################################################################################################
    print('\n2. PTQ Quantized: /////////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        ptq_net = q_mobilenet_v2(weights=MobileNet_V2_Weights.IMAGENET1K_V1)
    else:
        print('Finetuning for CIFAR dataset...')
        q_Mobilenet = q_mobilenet_v2(weights=MobileNet_V2_Weights.IMAGENET1K_V1)
        q_Mobilenet.classifier[-1] = nn.Linear(q_Mobilenet.classifier[-1].in_features, 10)
        q_Mobilenet = q_Mobilenet.to(DEVICE)
        optimizer = optim.SGD(q_Mobilenet.parameters(), lr=0.01, momentum=0.5)
        criterion = nn.CrossEntropyLoss()
        q_Mobilenet.train()
        train(q_Mobilenet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'quantizable'], load=True, device=DEVICE)
        ptq_net = deepcopy(q_Mobilenet)
...
##### QAT ###############################################################################################################################
    print('\n4. QAT Quantized: /////////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        qat_net = q_mobilenet_v2(weights=MobileNet_V2_Weights.IMAGENET1K_V1)
    else:
        qat_net = deepcopy(q_Mobilenet)
    ...
    print('\n6. Custom in-QAT: ///////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        qat_net_2 = q_mobilenet_v2(weights=MobileNet_V2_Weights.IMAGENET1K_V1)
    else:
        qat_net_2 = deepcopy(q_Mobilenet)
