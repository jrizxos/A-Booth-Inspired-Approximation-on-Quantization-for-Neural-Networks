...
from torchvision.models import mnasnet, mnasnet1_0, MNASNet1_0_Weights
...
class QuantizableInvertedResidual(mnasnet._InvertedResidual):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.apply_residual:
            return self.skip_add.add(input, self.layers(input))
        else:
            return self.layers(input)

mnasnet._InvertedResidual = QuantizableInvertedResidual
...
##### Pretrained Network ################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        model_name = 'MNASNet_CIFAR'
        MNASNet.classifier[-1] = nn.Linear(MNASNet.classifier[-1].in_features, 10)
        MNASNet = MNASNet.to(DEVICE)
        optimizer = optim.SGD(MNASNet.parameters(), lr=0.01, momentum=0.5)
        criterion = nn.CrossEntropyLoss()
        MNASNet.train()
        train(MNASNet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'main'], load=True, device=DEVICE)
    MNASNet.eval()
    test_nn(MNASNet, validation_loader, device=DEVICE)
...
##### PTQ ###############################################################################################################################
    print('\n2. PTQ Quantized: /////////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        ptq_net = QuantizableModel(mnasnet1_0(weights=MNASNet1_0_Weights.IMAGENET1K_V1))
    else:
        ptq_net = QuantizableModel(deepcopy(MNASNet))
...
##### QAT ###############################################################################################################################
    print('\n4. QAT Quantized: /////////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        qat_net = QuantizableModel(mnasnet1_0(weights=MNASNet1_0_Weights.IMAGENET1K_V1))
    else:
        qat_net = QuantizableModel(deepcopy(MNASNet))
    ...
    print('\n6. Custom in-QAT: ///////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        qat_net_2 = QuantizableModel(mnasnet1_0(weights=MNASNet1_0_Weights.IMAGENET1K_V1))
    else:
        qat_net_2 = QuantizableModel(deepcopy(MNASNet))
