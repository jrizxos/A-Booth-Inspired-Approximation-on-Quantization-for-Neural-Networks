...
##### Pretrained Network ################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        GoogLeNet.train()
        train(GoogLeNet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'main'], load=True, device=DEVICE)
    GoogLeNet.eval()
    test_nn(GoogLeNet, validation_loader, device=DEVICE)
...
##### PTQ ###############################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        criterion = nn.CrossEntropyLoss()
        q_GoogLeNet.train()
        train(q_GoogLeNet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'quantizable'], load=True, device=DEVICE)
