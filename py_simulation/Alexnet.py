...
##### Pretrained Network ################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        Alexnet.classifier[-1] = nn.Linear(Alexnet.classifier[-1].in_features, 10)
        Alexnet = Alexnet.to(DEVICE)
        optimizer = optim.SGD(Alexnet.parameters(), lr=0.01, momentum=0.5)
        criterion = nn.CrossEntropyLoss()
        Alexnet.train()
        train(Alexnet, train_loader, optimizer, criterion, epochs=10, name=[model_name,'main'], load=True, device=DEVICE)
    Alexnet.eval()
    test_nn(Alexnet, validation_loader, device=DEVICE)
...
##### PTQ ###############################################################################################################################
    print('\n2. PTQ Quantized: /////////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        ptq_net = QuantizableModel(alexnet(weights=AlexNet_Weights.IMAGENET1K_V1))
    else:
        ptq_net = QuantizableModel(deepcopy(Alexnet))
    ...
    print('\n3. Custom PTQ: ////////////////////////////////////////////////////////////\n',time.asctime())    
    time_1 = time.perf_counter()       
    apply_custom_quantization(ptq_net, name=[model_name,'PTQ_cus'], load=True)
    test_nn(ptq_net, validation_loader, device='cpu')
    time_2 = time.perf_counter()
    print('(in', time_2 - time_1,'seconds)\n')    
...
##### QAT ###############################################################################################################################
    print('\n4. QAT Quantized: /////////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        qat_net = QuantizableModel(alexnet(weights=AlexNet_Weights.IMAGENET1K_V1))
    else:
        qat_net = QuantizableModel(deepcopy(Alexnet))
    ...
    print('\n5. Custom post-QAT: ///////////////////////////////////////////////////////\n',time.asctime())
    time_1 = time.perf_counter()
    apply_custom_quantization(qat_net, name=[model_name,'QAT_post'], load=True)
    test_nn(qat_net, validation_loader, device='cpu')
    time_2 = time.perf_counter()
    print('(in', time_2 - time_1,'seconds)\n')    
    ...
    print('\n6. Custom in-QAT: ///////////////////////////////////////////////////////\n',time.asctime())
    if(IMAGENET_DATASET):
        qat_net_2 = QuantizableModel(alexnet(weights=AlexNet_Weights.IMAGENET1K_V1))
    else:
        qat_net_2 = QuantizableModel(deepcopy(Alexnet))
    ...
    time_1 = time.perf_counter()
    QAT_quantize(qat_net_2, train_loader, optimizer, criterion, epochs=3, name=[model_name,'QAT_in_pre'], qconfig_dict=qat_qconfig_dict, load=True, device=DEVICE)
    test_nn(qat_net_2, validation_loader, device='cpu')
    apply_custom_quantization(qat_net_2, name=[model_name,'QAT_in'], load=True)
    test_nn(qat_net_2, validation_loader, device='cpu')
    time_2 = time.perf_counter()
    print('(in', time_2 - time_1,'seconds)\n')    
