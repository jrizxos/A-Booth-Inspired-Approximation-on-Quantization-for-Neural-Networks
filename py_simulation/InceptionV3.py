...
##### Pretrained Network ################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        InceptionV3.train()
        train(InceptionV3, train_loader, optimizer, criterion, epochs=10, name=[model_name,'main'], load=False, device=DEVICE)
    InceptionV3.eval()
    test(InceptionV3, validation_loader, device=DEVICE)
...
##### PTQ ###############################################################################################################################
    ...
    if(IMAGENET_DATASET):
        ...
        criterion = nn.CrossEntropyLoss()
        q_InceptionV3.train()
        train(q_InceptionV3, train_loader, optimizer, criterion, epochs=10, name=[model_name,'quantizable'], load=False, device=DEVICE)
